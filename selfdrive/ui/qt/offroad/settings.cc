...
SoftwarePanel::SoftwarePanel(QWidget* parent) : QWidget(parent) {
  ...
  connect(updateBtn, &ButtonControl::clicked, [=]() {
    ...
    if (!last_ping.length()) {
      desc += QString("No internet connection. Connect to WiFi to check for update.");
    } else if (commit_local == commit_remote) {
      desc += QString("Local matches Remote. Update not required.");
    } else {
      desc += QString("Update Available. Click OK to apply.");
    }
  ...
  QObject::connect(presettwoload_btn, &QPushButton::clicked, [=]() {
    if (ConfirmationDialog::confirm("Are you sure you want to load Preset 2?", this)) {
  ...
  auto paraminit_btn = new ButtonControl("Initialize Parameters", "EXECUTE");
  QObject::connect(paraminit_btn, &ButtonControl::clicked, [=]() {
    if (ConfirmationDialog::confirm("Initialize parameters. Various changed values in the device menu will reset to initial values. Are you sure you want to proceed?", this)){
  ...
  QObject::connect(gitresetbtn, &ButtonControl::clicked, [=]() {
    if (ConfirmationDialog::confirm("After initializing local changes, the latest commit history of Remote Git is applied. Are you sure you want to proceed?", this)){
...
UserPanel::UserPanel(QWidget* parent) : QWidget(parent) {
  ...
  layout->addWidget(horizontal_line());
  layout->addWidget(new LabelControl("User", ""));
  ...
  QObject::connect(calokbtn, &ButtonControl::clicked, [=]() {
    if (ConfirmationDialog::confirm("Enable Force Calibration. Used for checking lane recognition. Enable during onroad.", this)){
      std::system(cal_ok);
    }
  });
  ...
  layout->addWidget(horizontal_line());
  layout->addWidget(new LabelControl("Panda Safety Values", ""));
  ...
  auto peditbtn = new ButtonControl("Apply Panda Value Changes", "EXECUTE");
  QObject::connect(peditbtn, &ButtonControl::clicked, [=]() {
    if (ConfirmationDialog::confirm("Apply the changed panda values. Are you sure you want to proceed? The device will automatically reboot after flashing panda.", this)){
...
TuningPanel::TuningPanel(QWidget* parent) : QWidget(parent) {
  ...
  layout->addWidget(new LabelControl("Lateral Tuning Menu", ""));
